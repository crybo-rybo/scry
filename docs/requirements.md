...
| QA-005 | MUST | ASan, UBSan, and TSan suites pass on every pull request; threaded tests always run under TSan. The fuzz corpus replay (QA-015) is a third per-pull-request sanitizer leg. The reflection component's marshalling and Scry-owned JSON bridge run under ASan+UBSan on every reflection-affecting pull request (path-aware gate) and unconditionally in the scheduled ring — this rerun is the component's only sanitizer coverage, because the core sanitizer legs build with reflection OFF. |
...
| QA-010 | SHOULD | The scheduled weekly ring runs deep static analysis (CodeQL), long fuzz runs on all five fuzz targets (SSE, Anthropic wire JSON, OpenAI-compatible wire JSON, HTTP response policy, and the `Conversation` document), the showcase gate, and the reflection component gate. The end-to-end smoke against a real local model runs on manual dispatch only, against a checksum-pinned server. |
...
| QA-015 | MUST | Every fuzz target builds and replays its checked-in seed corpus on every pull request under ASan and non-recovering UBSan; each seed is an input its target accepts at least partially. Long-budget runs remain in the scheduled ring. |
...
|---|---|---|
| API | Public-header audit (`cmake/CheckPublicHeaders.cmake`), `public_api_contract` static assertions, runtime + integration suites, the `Conversation` document fuzz target (`scry_conversation_fuzz`), package-consumer audits | API-005, API-009 (design constraints) |
...
| NET | Transport policy/curl/loopback suites and fake-transport integration suites; fuzz for hostile-input robustness, including the response-header and body policy target (`scry_response_policy_fuzz`); NET-002 additionally via clang-tidy and the sanitizer legs | — |
