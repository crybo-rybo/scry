...
| QA-013 | MUST | Every exported API and every maintained C++ header under `include/` and `src/` carries sufficient Doxygen documentation for its role and meaningful declarations. The HTML site provides distinct API and Source Documentation entry points, indexes maintained C++ files under those two trees only, and builds on pull requests and every push to `main`. Successful non-pull-request runs on `main` MUST deploy the site to GitHub Pages; pull-request runs MUST NOT deploy it. The documentation toolchain MUST remain build-only and outside installed/exported package metadata. Comment coverage and quality MUST remain pull-request review obligations and MUST NOT become a Doxygen completeness or warning gate. |
