...
**Blocking-mode escape hatch:** a synchronous `send`-and-wait exists for CLI tools and tests, implemented *on top of* the async machinery (pump-until-complete internally), never as a second code path. Being a pump loop is observable: it runs `update()` until the waited turn terminates, so callbacks and app-thread tool handlers belonging to every other accepted turn run inside the call; the waited turn exposes no `Turn` handle and so cannot be cancelled by the caller, ending only through completion, a terminal error, or Harness destruction; and calling it from inside a callback is rejected with `invalid_state`.
