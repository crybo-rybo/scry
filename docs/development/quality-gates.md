...
1. **Pull request (fast):** one merged hygiene job (generated Doxygen API and source
...
The reference site is generated by Doxygen 1.9.8 or newer with Graphviz `dot`,
through `scripts/ci-docs.sh`. Its API entry point curates the installed headers;
its Source Documentation entry point maps the contributor-facing implementation,
including private header contracts and browsable translation units. The code
index is restricted to maintained C++ files under `include/` and `src/`; tests,
benchmarks, examples, and other code trees are excluded. Doxygen diagnostics
remain visible in the build log for reviewer judgment, while CI requires only
that generation complete and produce the site. The generated HTML is retained
as a CI artifact. A successful non-pull-request run on `main` publishes the same
artifact to GitHub Pages; pull requests verify the site but cannot deploy it.
Doxygen and Graphviz are build-only documentation tools: neither participates
in the normal CMake project, installed package, exported targets, or consumer
dependency graph. This separate entry point keeps a docs toolchain failure from
changing the C++23 library configuration surface while remaining part of
`preflight.sh` and the definition of done (QA-013).
