...
cannot provide; hosted CI is authoritative for those environments. A leg the
host cannot provide is reported as a skip rather than a failure, and the run's
closing summary names every skipped leg, so the set of hosted-only gates for a
change is explicit rather than inferred from scrollback. On macOS the GCC 14
core leg and the version-pinned clang-tidy 18 leg both run locally once
`brew install gcc@14 llvm@18` has provided them — the two hosted legs that most
often catch what AppleClang accepts — and are reported as skipped when they are
absent. The scheduled-ring legs (CodeQL, long fuzz, showcase) and the manual
local-model smoke are not part of the one-command ring. A gate with no local
entry point is a gate you learn about only by pushing, which breeds
workarounds.
