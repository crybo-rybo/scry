...
scheduled-ring legs (CodeQL, long fuzz) and the manual local-model smoke are
not part of the one-command ring. A gate with no local entry point is a gate
you learn about only by pushing, which breeds workarounds.
