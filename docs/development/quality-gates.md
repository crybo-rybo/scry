...
retained as a CI artifact. A successful non-pull-request run on `main` publishes
the same artifact to GitHub Pages; pull requests verify the site but cannot
deploy it. Doxygen and Graphviz are build-only documentation tools: neither
participates in the normal CMake project, installed package, exported targets,
or consumer dependency graph. This separate entry point keeps a docs toolchain
failure from changing the C++23 library configuration surface while remaining
part of `preflight.sh` and the definition of done (QA-013).
