...
clang-tidy covers the library and the examples, not the tests. Rows below are
...
| ~~27~~ | ~~clang-tidy is attached only to the core target. Tests, examples, and the reflection component are never analyzed, although the config names them.~~ Closed by decision: clang-tidy covers the library and the examples, not the tests. The tidy build now compiles only the library and the fuzz targets. | ~~`CMakeLists.txt` `CXX_CLANG_TIDY`~~ |
