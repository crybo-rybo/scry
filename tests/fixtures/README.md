...
OpenAI Chat Completions streaming references, except the two
`request_tool_history.json` goldens, which were captured from Scry's own
encoder. None of them was captured from a live service.** They were written to match the documented wire shapes, then
...
| `anthropic/request_tool_history.json`, `openai/request_tool_history.json` | Each dialect's body for one request carrying text, two tool calls with nested-object arguments, a plain and an error tool result, and two tools with nested schemas. Unlike the other goldens these were captured from Scry itself: they are what the generic-tree encoder emitted before the encoders were changed to splice stored canonical text, so they pin that the rewrite is byte-for-byte equivalent | `tests/provider/request_encoding_tests.cpp` |
