...
Third of three: [DESIGN.md](DESIGN.md) says what Scry is, [ARCHITECTURE.md](ARCHITECTURE.md) says how the code is shaped, this document says **how the project is engineered** — the quality machinery, gates, and habits that govern every change to the library. Philosophy over procedure; specific thresholds appear only where a gate needs a number to be enforceable.
...
- clang-tidy with a curated, checked-in profile (bugprone-*, concurrency-*, cppcoreguidelines-* selectively, modernize-*, performance-*, readability-*). Curated means every disabled check has a one-line reason in the config — the config is documentation of the project's taste.
